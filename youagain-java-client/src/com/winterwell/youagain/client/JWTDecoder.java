...
public class JWTDecoder {
	...
	public void verify(String jwt, String issuer) {
		...
		if (jwt.isEmpty()) throw new IllegalArgumentException("blank jwt '"+jwt+"\'");
		Algorithm algorithm = algorithm();
		
		assert ! jwt.endsWith("temp") : jwt;
		JWTVerifier verifier; // should this be static for speed? it says its thread safe. Does speed matter to us here??
		try {
		...
		try {
			...
			if (debug) {
				...
				if ( ! Utils.equals(issuer, decoded.getIssuer())) {
					Log.w(LOGTAG, "(ignore) verify - issuer mismatch! expected: "+issuer+" got: "+decoded.getIssuer()+" from "+jwt+" "+ReflectionUtils.getSomeStack(12));
				}
			}
		} catch(Throwable ex) {
			Object token = "";
