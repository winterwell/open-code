...
public class AggregationTest {
	...
	@Test
	public void testSourceUnset() {
		ESHttpClient esjc = new ESHttpClient(new ESConfig());
		SearchRequest s = esjc.prepareSearch("datalog.emissions.all");
		Aggregation agg = Aggregations.terms("by_source", "source");
		agg.setMissing("unset");
		s.addAggregation(agg);
		s.setDebug(true);
		SearchResponse res = s.get();
	}
