...
//		init3_youAgain();		
	...
	protected void doMain2() {		
		...
		TransformRequest trb = esc.transform().prepareTransform(jobId);
		...
		TransformRequest trb2 = esc.transform().prepareTransformStart(jobId); 
		trb2.setDebug(true);
	 ...
	 * {@link ESDataLogIndexManager#registerDataspace()} which does much the same??
	 ...
	 * @return
	 */
	public void createIndexWithPropertiesMapping(String idx, String alias, List<String> terms) {
		ESHttpClient esc = Dep.get(ESHttpClient.class);
		// already exists?
