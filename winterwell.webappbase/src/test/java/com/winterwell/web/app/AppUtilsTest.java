...
public class AppUtilsTest {
	...
	@Test
	public void testValueWithColon() { // a smoke test
		String q = "segment:\"T4G:register\"";
		SearchQuery sq = new SearchQuery(q);
		BoolQueryBuilder esf = AppUtils.makeESFilterFromSearchQuery(sq);
...
//		System.out.println(esmap);
	...
	@Test
	public void testMakeESFilterFromSearchQueryCHF() {		
		{
			String q = "CHF2USD:any";
			SearchQuery sq = new SearchQuery(q);
			List ptre = sq.getParseTree();
			BoolQueryBuilder esf = AppUtils.makeESFilterFromSearchQuery(sq, null, null);
			Map esmap = (Map) esf.getUnderlyingMap().get("bool");
...
//			assert esmap.containsKey("must") : esmap;
	...
	public void testMakeESFilterFromSearchQuery() {	
		{
			String q = "cid:nestle AND evt:spend";
			SearchQuery sq = new SearchQuery(q);
			List ptre = sq.getParseTree();
			BoolQueryBuilder esf = AppUtils.makeESFilterFromSearchQuery(sq, null, null);
			Map esmap = (Map) esf.getUnderlyingMap().get("bool");
		...
		}
		{
			...
			SearchQuery sq = new SearchQuery(q);
			List ptre = sq.getParseTree();
			System.out.println(ptre);
			assert ptre.toString().equals(
			 "[and, [or, {cid=nestle-cocoa-plan-vegetable-growing-kit}, {cid=tchc}, {cid=unset}], [or, {evt=spend}, {evt=spendadjust}, {evt=donation}]]"		
			) : ptre.toString();
			BoolQueryBuilder esf = AppUtils.makeESFilterFromSearchQuery(sq, null, null);
			Map esmap = (Map) esf.getUnderlyingMap().get("bool");
			System.out.println(esmap);
