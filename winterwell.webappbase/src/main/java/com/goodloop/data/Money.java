...
//			.property("value100", new ESType().DOUBLE()) 
	...
	/**
	 * A non-currency zero. 
	 * You can also safely use zeros with an arbitrary currency
	 *  -- adding / subtracting zero is a no-op and currency is ignored.
	 */
	public static final Money ZERO = new Money(null, 0);
...
//	@ESKeyword TODO probably
	...
	public Money plus(Money x) {
		...
		// currency must match (0s have already been screened)
	 ...
	 * 
	 * @param x
	 * @return a new Mony for this * x, or this if x=0
	 */
	public Money minus(Money x) {
		if (x.isZero()) return this;
		return plus(x.multiply(-1));
	}
