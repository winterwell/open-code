...
import com.winterwell.web.WebEx.RateLimitException;
...
public class FakeBrowser {
	...
	/**
	 * Otherwise starts with an instant retry
	 * @param minPause
	 */
	...
	private void processResponse2_errorCodes()
		...
		if (code >= 400 && code < 500)  {			
			...
			if (code==429) {
				RateLimitException rle = new RateLimitException(errorMessage);
				String ra = Containers.first(headers.get("Retry-After"));
				if (ra != null) {
					rle.setRetryAfter(ra);
				}
				throw rle;
			}			
			if (code==431) {
	...
	/**
	 * @param retries e.g. 2 retries = 3 tries in total
	 * @see #setMinRetryPause(Dt)
	 */
